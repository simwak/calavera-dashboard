<template>
  <v-dialog v-model="show" max-width="600px" >
    <v-card>
      <v-card-title>
        <span class="headline">Modify editor password</span>
      </v-card-title>
      <v-card-text>
        ...
        <span>Modifying the editor will cause the editor to restart, which can lead to data loss.</span>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
  ...
  export default {
    ...
    methods: {
      async changePassword() {
        try {
          var response = await this.axios.patch('https://' + this.$config.domains.api + '/editor/password', { password: this.password })

          if(response.data.status == 'successfull') {
            await this.axios.post('https://' + this.$config.domains.api + '/editor/stop')
            await this.axios.post('https://' + this.$config.domains.api + '/editor/start')
